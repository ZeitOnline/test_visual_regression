...
class Agatho(object):
    ...
    def collection_get(self, unique_id):
        ...
        try:
            # return etree.parse('%s%s%s' % (
            #     self.entry_point, path_of_article(unique_id), random))
            return etree.parse(
...
def get_thread(unique_id, request):
    """return a dict representation of the comment thread of the
    given article"""
    agatho_url = request.registry.settings.get('agatho_url')
    if not agatho_url:
        return None

    api = Agatho(agatho_url)
