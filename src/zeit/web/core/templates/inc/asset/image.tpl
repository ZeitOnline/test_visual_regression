    ...
    <figure class="{% block media_block %}{{ module_layout }}__media{% endblock %} {{ media_block_additional_class }} scaled-image"
        ...
        <!--<![endif]-->
            <div class="{% block media_block_helper %}{{ module_layout }}__media-container{% endblock %} {{ media_container_additional_class }}">
                ...
                <img class="{% block media_block_item %}{{ module_layout }}__media-item{% endblock %}" alt="{{ image.alt }}" src="{{ fallback_source }}" data-src="{{ source }}" data-ratio="{{ image.ratio }}" {% block media_block_additional_data_attributes %}
{% if image.mobile_ratio %}
data-mobile-src="{{ source }}" data-mobile-ratio="{{ image.mobile_ratio }}"
{% endif %}
{% endblock %}>
                {% endblock %}
