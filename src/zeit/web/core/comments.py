...
class Community(grokcore.component.GlobalUtility):

    grokcore.component.implements(zeit.web.core.interfaces.ICommunity)

    def get_thread(self, unique_id, sort='asc', page=0,
        ...
        pages = int(math.ceil(
            float(pagination_comment_count) / float(page_size)))
    ...
    def get_comment(self, unique_id, cid):
        ...
        try:
            ...
            log.warning(
                'get_comment input unparseable, ignoring', exc_info=True)
            return {}
    ...
    def get_comment_count(self, unique_id):
        ...
        try:
            ...
            log.warning(
                'get_comment_count input unparseable, ignoring', exc_info=True)
            return 0
    ...
    def get_comment_counts(self, *unique_ids):
        ...
        :rtype: dict
        """
        raw = self._request_counts(*unique_ids)
        if raw is None:
            return {}
        try:
            ...
            log.warning(
                'get_counts input unparseable, ignoring', exc_info=True)
            return {}

    def get_user_comments(self, author, page=1, rows=6, sort='DESC'):
        ...
        uri = '{}/agatho/usercommentsxml/{}/{}/{}/{}'.format(
            conf.get('agatho_host', ''), author.email.encode('utf-8'),
            (page - 1), rows, sort)

        timeout = float(conf.get('community_host_timeout_secs', 5))

        response = None
        with zeit.web.core.metrics.http('user_comments.community') as record:
            try:
    ...
    def is_healthy(self):
        ...
        try:
            with zeit.web.core.metrics.http(
                    'health_check.community') as record:
                response = requests.get(uri, timeout=timeout)
                record(response)
    ...
    def _request_thread(self, unique_id, thread_type='full',
        ...
        thread_modes = dict(
            ...
            promotion=(
                ...
                '&recommendationtype=kommentar_empfohlen'
        ...
        try:
            with zeit.web.core.metrics.http(
                    'request_thread.community') as record:
                response = requests.get(uri, timeout=timeout)
                record(response)
        except Exception:
            log.warning(
    ...
    def _request_counts(self, *unique_ids):
        ...
        try:
            with zeit.web.core.metrics.http(
                    'request_counts.community') as record:
                response = requests.post(
                    uri, data=[('unique_ids[]', uid) for uid in unique_ids],
                    timeout=timeout)
                record(response)
