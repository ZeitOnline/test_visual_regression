...
def test_inline_gallery_should_have_images(application):
    ...
    image = zeit.web.core.template.get_image(list(gallery)[4])
    assert image.path.endswith('/galerien/bg-automesse-detroit-2014-usa-bilder'
                               '/chrysler 200 s 1-540x304.jpg')
    assert image.alt is None
    assert image.copyrights[0][0] == u'\xa9'
