    ...
    <a class="nextread__link{% if has_default_image %} nextread__link--without-image{% endif %}" title="{{ teaser.supertitle }}: {{ teaser.title }}" href="{{ teaser.uniqueId | create_url }}">
        ...
        <div class="nextread__helper {% if has_default_image %}nextread__helper--without-image{% endif %}">
            <div class="nextread__inner-helper{% if has_default_image %} nextread__inner-helper--without-image{% endif %}">
                <div class="nextread__kicker{% if has_default_image %} nextread__kicker--without-image{% endif %}">{{ teaser.teaserSupertitle or teaser.supertitle | hide_none }}</div>
                <div class="nextread__title{% if has_default_image %} nextread__title--without-image{% endif %}">{{ teaser.teaserTitle or teaser.title | hide_none }}</div>
                <div class="nextread__metadata{% if has_default_image %}nextread__metadata--without-image{% endif %}">{{ cp.include_teaser_datetime(teaser, 'nextread') }}</div>
            </div>
        </div>
    </a>
</div>
