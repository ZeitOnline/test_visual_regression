{% macro main_nav(navigation, request, nav_class=None) -%}
    ...
    <ul class="{{ nav_class }}{% if nav_class == 'primary-nav' %} primary-nav--js-no-overflow{% endif %}">
        ...
        <li class="{{ nav_class }}__item" data-id="{{ section.item_id }}"{% if section.has_children() %} data-feature="dropdown"{% endif %}>
            ...
            {% if section.has_children() -%}
                {{ main_nav(section, request, 'dropdown') }}
            {%- endif %}
        </li>
        {%- endfor %}
        ...
        <li class="{{ nav_class }}__item" data-id="more-dropdown" data-feature="dropdown">
            ...
            {{ main_nav(navigation, request, 'dropdown') }}
        </li>
        <li class="primary-nav__item primary-nav__item--featured">
            <a class="primary-nav__link" href="http://{{ request.host }}/zeit-magazin/index" id="hp.global.topnav.centerpages.zeitmagazin">ZEITmagazin</a>
        </li>
        {% endif %}
    </ul>
{%- endmacro %}

{% macro main_nav_ressorts(navigation, request, nav_class=None) -%}
    <nav role="navigation" id="primary_nav">
        {{ main_nav(navigation, request, 'primary-nav') }}
    </nav>
{%- endmacro %}

{% macro main_nav_services(navigation, request, nav_css=None) -%}
    {{ main_nav(navigation, request, 'primary-nav-services') }}
{%- endmacro %}

{% macro main_nav_classifieds(navigation, request) -%}
    {{ main_nav(navigation, request, 'main-nav-classifieds') }}
{%- endmacro %}

{% macro main_nav_community(request) -%}
    <a href="http://community.zeit.de/user/login?destination=http://{{ request.host }}/index" rel="nofollow" class="user" id="drupal_login">
        <span class="main_nav__community__image icon-zon-logo-navigation_login"></span>
        Anmelden
    </a>
{%- endmacro %}

{% macro main_nav_logo(request) -%}
...
{% macro main_nav_search(request) -%}
    {# TODO: Make action host configurable, as soon as friedbert knows search. #}
