...
def test_column_should_not_have_header_image(testbrowser):
    browser = testbrowser('/zeit-magazin/article/standardkolumne-ohne-bild-beispiel')
...
def test_article10_has_correct_date_formats(application):
    ...
    context = zeit.cms.interfaces.ICMSContent('http://xml.zeit.de/zeit-magazin/article/10')
    article_view = zeit.web.magazin.view_article.Article(context, mock.Mock())
    assert article_view.show_date_format == 'long'
...
def test_article_page_should_throw_404_if_no_pages_are_exceeded(application):
    article = zeit.cms.interfaces.ICMSContent('http://xml.zeit.de/zeit-magazin/article/03')
...
def test_pagination_dict_should_have_correct_entries(application):
    ...
    view.request.traversed = (u'article', u'03')
    ...
    view.request.traversed = ('article', '03')
    view.request.route_url.return_value = '/'
...
def test_pagination_urls_list_should_have_correct_entries_paged_article(
    ...
    article = zeit.cms.interfaces.ICMSContent('http://xml.zeit.de/zeit-magazin/article/03')
    view = zeit.web.magazin.view_article.ArticlePage(article, mock.Mock())
    view.request.path_info = u'zeit-magazin/article/03/seite-2'
    view.request.traversed = ('zeit-magazin/article', '03')
    view.request.route_url.return_value = '/'

    assert view.pages_urls[0] == '/zeit-magazin/article/03'
    assert view.pages_urls[1] == '/zeit-magazin/article/03/seite-2'
    assert view.pages_urls[2] == '/zeit-magazin/article/03/seite-3'
...
def test_pagination_urls_list_should_have_correct_entries_single_article(
    ...
    view = zeit.web.magazin.view_article.ArticlePage(article, mock.Mock())
    view.request.path_info = u'zeit-magazin/article/01'
    view.request.traversed = (u'zeit-magazin/article', u'01')
    view.request.route_url.return_value = '/'

    assert view.pages_urls[0] == '/zeit-magazin/article/01'
...
def test_pagination_next_page_url_is_working(application):
    ...
    assert view.pagination['next_page_url'] == '/zeit-magazin/article/03/seite-2'


def test_pagination_next_page_url_on_last_page_is_none(application):
    article = zeit.cms.interfaces.ICMSContent('http://xml.zeit.de/zeit-magazin/article/03')
    view = zeit.web.magazin.view_article.ArticlePage(article, mock.Mock())
    view.request.path_info = u'zeit-magazin/article/03/seite-7'
    view.request.traversed = (u'zeit-magazin/article', u'03')
    view.request.route_url.return_value = '/'
...
def test_pagination_prev_page_url_is_working(application):
    ...
    assert view.pagination['prev_page_url'] == u'/zeit-magazin/article/03'
...
def test_advertorial_is_advertorial(application):
    context = zeit.cms.interfaces.ICMSContent(
        'http://xml.zeit.de/zeit-magazin/centerpage/advertorial')
...
def test_adv_teaser_on_cp_should_render_modifier(testbrowser):
    browser = testbrowser('/zeit-magazin/centerpage/index')
    assert browser.cssselect('.is-advertorial')


def test_adv_teaser_on_adv_should_not_render_modifier(testbrowser):
    browser = testbrowser('/zeit-magazin/centerpage/advertorial')
